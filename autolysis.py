...
def detect_outliers(df):
    ...
    outliers = (z_scores > 3).sum()  # Count of outliers per column
    return outliers
...
def create_markdown_story(llm_response, output_dir):
    """Create a README markdown file with analysis results."""
...
def main():
    """Main function for data analysis and reporting."""
    if len(sys.argv) != 2:
    ...
    for column in df.columns[:5]:  # Plot up to 5 columns for demonstration
        plot_histogram(df, column, output_dir)
